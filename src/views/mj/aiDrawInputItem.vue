...
const shorten = () => {
  ...
  let obj = {
    action: "shorten",
    data: {
      prompt: st.value.text.trim(),
      botType: st.value.bot == "NIJI_JOURNEY" ? "NIJI_JOURNEY" : "MID_JOURNEY",
    },
  };
...
function createPrompt(rz: string) {
  ...
  for (let v of farr) {
    ...
    if (v.k == "quality") rzp += `  --q ${f.value.quality}`;
      ...
      if (f.value.styles) rzp += ` ${f.value.styles} `;
    } else if (v.k == "version") {
      st.value.bot = "";
      if (["MID_JOURNEY", "NIJI_JOURNEY"].indexOf(f.value.version) > -1) {
        st.value.bot = f.value.version;
...
<template>
  ...
  <div class="overflow-y-auto bg-[#fafbfc] px-4 dark:bg-[#18181c] h-full">
    ...
    <section class="mb-4 flex justify-between items-center" v-for="v in farr">
      ...
      <n-select
        v-model:value="f[v.k]"
        :options="drawlocalized[v.k + 'List']"
        size="small"
        class="!w-[60%]"
        :clearable="true"
      />
    ...
    <section class="mb-4 flex justify-between items-center">
      ...
      <NInput
        v-model:value="f.sref"
        size="small"
        placeholder="图片url 生成风格一致的图像"
        clearable
      >
        <template #suffix>
          <SvgIcon
    ...
    <section class="mb-4 flex justify-between items-center">
      ...
      <NInput
        v-model:value="f.cref"
        size="small"
        placeholder="图片url 生成角色一致的图像"
        clearable
      >
        <template #suffix>
          <SvgIcon
          ...
          <NPopover trigger="hover">
            <template #trigger>
              <n-tag
                ...
                :bordered="false"
                @click="fsRef.click()"
                v-if="st.fileBase64.length"
              >
                <div style="display: flex">
                  <SvgIcon icon="mdi:file-chart-check-outline" />
                  {{ $t("mjchat.imgCYes") }}
                </div>
              </n-tag>
              <n-tag
                ...
                :bordered="false"
                @click="fsRef.click()"
                v-else="st.fileBase64"
              >
                <div style="display: flex">
                  <SvgIcon icon="mdi:file-document-plus-outline" />
                  {{ $t("mjchat.imgCUpload") }}
                </div>
              </n-tag>
            </template>
              ...
              >
                <div class="p-1" v-for="v in st.fileBase64">
                  ...
                  <NButton
                    size="small"
                    @click="
                      st.fileBase64 = st.fileBase64.filter((item) => item != v)
                    "
                    type="warning"
                    >{{ $t("mjchat.del") }}</NButton
                  >
                </div>
              </div>
            </div>
        ...
        <div class="pr-1 pt-1">
          <NPopover trigger="hover">
            <template #trigger>
              ...
              >
                <div style="display: flex">
                  <SvgIcon icon="fluent:image-edit-16-regular" />
                  {{ $t("mjchat.img2text") }}
                </div>
              </n-tag>
            </template>
