...
import { ref } from "vue";
import { useMessage, NButton, NPopover } from "naive-ui";
...
<template>
  ...
  <div class="flex justify-around items-center">
    <div
      class="h-[80px] w-[80px] rounded-sm border border-gray-400/20 flex justify-center items-center cursor-pointer"
      @click="(st.status = 'source') && fsRef.click()"
    >
    ...
    <div
      class="h-[80px] w-[80px] rounded-sm border border-gray-400/20 flex justify-center items-center cursor-pointer"
      @click="(st.status = 'target') && fsRef.click()"
    >
  ...
  <div class="flex justify-center pt-5">
    <NPopover trigger="hover">
      <template #trigger>
        <NButton @click="send" type="primary" :disabled="true">{{
          $t("mjchat.submit")
        }}</NButton>
      </template>
  ...
  <ul class="pt-4" v-html="$t('mjchat.tipInfo')"></ul>
</template>